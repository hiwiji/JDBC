...
public class JDBCExample {
	public static void main(String[] agrs ) {
		...
		// 위에 3가지가 준비가 끝나면 자바와 db 통신할 준비 끝! 
		
		
		// 1단계 : JDBC 객체 참조 변수 선언 (java.sql 패키지의 인터페이스)
		
		
		Connection conn = null;          // Connection  java.sql로 import 해주기
		// DB연결 정보를 담은 객체
		...
		Statement stmt = null;			// Statement  java.sql로 import 해주기
		// Connection을 통해
		// SQL 문을 DB에 전달하여 실행하고
		// 생성된 결과(ResultSet, 성공한 행의 개수)를 반환(Java)하는데 사용되는 객체
		
		
		ResultSet rs = null;			// ResultSet  java.sql로 import 해주기
		// SELECT 질의 성공시 반환되는데
		// 조회 결과 집합을 나타내는 객체
		...
		try {
			...
			String type = "jdbc:oracle:thin:@"; // JDBC 드라이버의 종류 (오라클은 이렇게씀)
			
			String ip = "localhost"; // DB 서버 컴퓨터 IP
			// localhost == 127.0.0.1 (loop back ip)  
			// 위에 localhost 나 loop back ip 안될 경우 cmd 창에 ipconfig 쳐서 
			// IPv4 ip주소 넣어서 해보기
			...
			// getConnection부분엔 (URL, USER, PW) 창인데 
			// URL == type + ip + port + sid 합친거여서 위와같이 쓰면 됩니다~~~~
			...
			while(rs.next()) {
				...
				Date hireDate = rs.getDate("HIRE_DATE");     // Date import 해주기 sql로
				// -> java.sql.Date
