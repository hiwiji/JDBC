...
public class MainView {
	...
	// -> 로그인 X == null
	// -> 로그인 O != null 로그인 하면 로그인 값이 여기에 담겨있지요~_~
	...
	private void login() {
		...
		try {
			...
			LoginMember = service.login(memberId, memberPw);  // Mainservice에서 throws해서 오류남
			// 서비스에 있는 로그인 메서드로 보냄 			
			// 여기서 던지는거 종료하려면  try - catch문으로 감싸주기 
	...
	private void signUp() {
		...
		try {
			...
			while(true) {
				...
				// 중복 여부를 검사하는 서비스 호출
				// 후 결과 반환 받기 (있으면 1/ 없으면 0)
				
				int result = service.idDupCheck(memberId); // 입력받은 매개변수 서비스로 전달~~
				// 1/ 0
				...
				if(result == 0) {
					...
					break; // 가장 가까운 반복문 탈출. 반복멈춰서 밑으로 빠져나감 
			...
			System.out.println("\n<<회원 가입 중 예외 발생>>\n");
			e.printStackTrace(); 
			// 에러의 발자국. 어디에서 뭐났는지 알려주니 꼭 적어주세요~~
		}
