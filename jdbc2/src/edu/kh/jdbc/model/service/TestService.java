...
//import Static 구문 
//-> static이 붙은 필드, 메서드를 호출 할 때
//클래스명을 생략할 수 잇게 하는 구문
...
// 이를 하나의 트랜잭션 단위로 취급하여              // 아메를30개주문
// 한번에 commit, rollback을 수행할 수 있다.     //30개 잘됬으면 커밋. 아니면 롤백
...
public class TestService {
	...
	public int insert(TestVO vo1) throws SQLException {
		...
		//Connection conn = JDBCTemplate.getConnection();	//클래스명.메서드명(JDBCTemplate)에서 만든것
	...
	public int insert(TestVO vo1, TestVO vo2, TestVO vo3) throws Exception {
		...
		try {
			...
			if(result1 + result2 + result3 == 3 ) { // 모두 insert 성공한 경우
				...
				rollback(conn);
				// res = 0; 위에 초기값이 0이기 때문에 굳이 안써줘도 됩니다
			}
			
		} catch(SQLException e) { // dao 수행 중에 예외 발생 시
			rollback(conn);
	...
	public int update(TestVO vo) throws SQLException {
		...
		//커넥션, vo 같이 전달  
		// update부분 밑줄나오면 create 만들기(tese Dao클래스 만들어짐)
